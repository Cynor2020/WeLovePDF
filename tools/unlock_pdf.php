...
ini_set('error_log', __DIR__ . '/pdf_unlock_errors.log');
...
if (isset($_GET['download']) && isset($_GET['file'])) {
    ...
    $unlockedDir = __DIR__ . '/unlocked/';
    $file = basename($_GET['file']);
    $filePath = $unlockedDir . $file;
    
    if (file_exists($filePath)) {
        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="unlocked_' . $file . '"');
...
// Handle PDF upload and unlocking
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Start output buffering
    ob_start();
    
    try {
        error_log("Starting PDF unlock process");
        ...
        error_log("Checking directories: $uploadDir, $unlockedDir");
        if (!@mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
            throw new Exception('Failed to create uploads directory');
        }
        if (!@mkdir($unlockedDir, 0755, true) && !is_dir($unlockedDir)) {
            throw new Exception('Failed to create unlocked directory');
        }
        ...
        $pdf = null;
        $useTcpdf = class_exists('setasign\Fpdi\Tcpdf\Fpdi');
        if ($useTcpdf) {
        ...
        // Attempt to set source file with password
        error_log("Attempting to open PDF with password: $uploadPath");
        try {
            $pageCount = $pdf->setSourceFile($uploadPath, $password);
            error_log("Successfully opened PDF. Page count: $pageCount");
        } catch (Exception $e) {
            error_log("Password error: " . $e->getMessage() . " in " . $e->getFile() . " on line " . $e->getLine());
            // If TCPDF failed, try basic FPDI as a fallback
            if ($useTcpdf && class_exists('setasign\Fpdi\Fpdi')) {
                error_log("Falling back to basic FPDI");
                $pdf = new \setasign\Fpdi\Fpdi();
                try {
                    $pageCount = $pdf->setSourceFile($uploadPath, $password);
                    error_log("Successfully opened PDF with basic FPDI. Page count: $pageCount");
                } catch (Exception $e2) {
                    error_log("Fallback error: " . $e2->getMessage() . " in " . $e2->getFile() . " on line " . $e2->getLine());
                    unlink($uploadPath);
                    throw new Exception('Your PDF password is wrong, kindly enter a correct password');
                }
            } else {
                unlink($uploadPath);
                throw new Exception('Your PDF password is wrong, kindly enter a correct password');
        ...
        // Save unlocked PDF (no protection)
        $unlockedFilename = 'unlocked_' . $fileName;
        $unlockedPath = $unlockedDir . $unlockedFilename;
        error_log("Saving unlocked PDF to: $unlockedPath");
        $pdf->Output($unlockedPath, 'F');
        
        // Clean up original file
        unlink($uploadPath);
        
        // Clean up old unlocked files (older than 24 hours)
        error_log("Cleaning up old unlocked files");
        $files = glob($unlockedDir . '*.pdf');
        ...
        $response = [
            ...
            'downloadUrl' => '?download=true&file=' . $unlockedFilename
        ];
        
    } catch (Exception $e) {
        // Log detailed error
        error_log("Error in unlock_pdf.php: " . $e->getMessage() . " in " . $e->getFile() . " on line " . $e->getLine());
...
<head>
    ...
    <title>PDF Password Unlock</title>
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-md p-6">
            <h1 class="text-2xl font-bold text-center mb-6">
                <span class="gradient-text">Unlock PDF</span> with Password
            </h1>
            
            <form id="pdfUnlockForm" enctype="multipart/form-data">
                <!-- PDF Upload -->
                <div id="upload-area" class="upload-area rounded-lg p-8 text-center cursor-pointer mb-6"
                    ...
                    <h3 class="text-lg font-bold mb-2">Select Protected PDF File</h3>
                ...
                <!-- Password Input -->
                <div class="mb-6">
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-2">Enter PDF Password</label>
                    <div class="relative">
                ...
                <!-- Submit Button -->
                <button type="submit" id="unlock-btn" class="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-lg font-medium">
            ...
            <div id="result-container" class="hidden mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                <div class="flex items-center">
                    ...
                    <div>
                        <h3 class="font-bold text-green-800">PDF Unlocked Successfully!</h3>
    ...
    <script>
        // DOM Elements
        const form = document.getElementById('pdfUnlockForm');
                ...
                if (!response.ok) {
                    return response.text().then(text => {
                        ...
                        try {
                            ...
                            throw new Error(json.message || 'Server error: No response body');
                        } catch (e) {
