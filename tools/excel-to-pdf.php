...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['excelFile'])) {
    ...
    die(json_encode(['success' => false, 'message' => 'File upload failed with error code: ' . $excelFile['error']]));
}



        $fileExt = strtolower(pathinfo($excelFile['name'], PATHINFO_EXTENSION));
        $allowedTypes = ['xls', 'xlsx', 'csv', 'ods'];
        
        if (!in_array($fileExt, $allowedTypes)) {
            die(json_encode(['success' => false, 'message' => 'Invalid file type. Only Excel files (.xls, .xlsx, .csv, .ods) are allowed.']));
        }

        // Load spreadsheet with optimal settings
        $spreadsheet = IOFactory::load($excelFile['tmp_name']);
        
        // Configure ALL worksheets for perfect PDF output
        ...
        if (file_exists($outputFile)) {
            ...
            header('Content-Disposition: attachment; filename="' . 
                  pathinfo($excelFile['name'], PATHINFO_FILENAME) . '.pdf"');
...
<head>
    ...
    <title>Excel to PDF - WeLovePDF</title>
    <!-- Tailwind CSS -->
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-4xl mx-auto">
                <h1 class="text-3xl md:text-4xl font-bold mb-6 text-center">
                    Convert <span class="gradient-text">Excel to PDF</span>
                </h1>
                <p class="text-lg text-gray-600 mb-8 text-center">
                    Upload your Excel file and convert it to a high-quality PDF with all content preserved.
                </p>
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    <form id="converterForm" method="post" enctype="multipart/form-data" class="space-y-6">
                        <div id="uploadContainer" class="upload-area border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer"
                            ...
                            <input type="file" id="excelFile" name="excelFile" accept=".xls,.xlsx,.csv,.ods" class="hidden">
                            <div class="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i class="fas fa-file-excel text-3xl text-blue-500"></i>
                            </div>
                            <h3 class="text-xl font-bold mb-2">Select Excel File</h3>
                            ...
                            <p class="text-sm text-gray-400 mt-4">Supports .xls, .xlsx, .csv, .ods files up to 50MB</p>
                        </div>
