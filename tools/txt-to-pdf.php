...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['textFile'])) {
    try {
        ...
        if ($textFile['error'] !== UPLOAD_ERR_OK) {
            die(json_encode(['success' => false, 'message' => 'File upload failed with error code: ' . $textFile['error']]));
        }

        $fileExt = strtolower(pathinfo($textFile['name'], PATHINFO_EXTENSION));
        $allowedTypes = ['txt', 'text', 'log', 'csv'];
        
        if (!in_array($fileExt, $allowedTypes)) {
            die(json_encode(['success' => false, 'message' => 'Invalid file type. Only text files (.txt, .text, .log, .csv) are allowed.']));
        }

        // Read text file content
        $fileContent = file_get_contents($textFile['tmp_name']);
        
        // Create a new Spreadsheet
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        
        // Split text into lines and write to spreadsheet
        $lines = explode("\n", $fileContent);
        foreach ($lines as $row => $line) {
            $sheet->setCellValue('A' . ($row + 1), $line);
        }
        
        // Configure worksheet for PDF output
        ...
        $outputFile = $uploadDir . uniqid() . '.pdf';
        ...
        if (file_exists($outputFile)) {
            ...
            header('Content-Disposition: attachment; filename="' . 
                  pathinfo($textFile['name'], PATHINFO_FILENAME) . '.pdf"');
...
<head>
    ...
    <title>Text to PDF - WeLovePDF</title>
    <!-- Tailwind CSS -->
...
<body class="bg-gray-50 text-gray-800">
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-4xl mx-auto">
                <h1 class="text-3xl md:text-4xl font-bold mb-6 text-center">
                    Convert <span class="gradient-text">Text to PDF</span>
                </h1>
                <p class="text-lg text-gray-600 mb-8 text-center">
                    Upload your text file and convert it to a clean, readable PDF document.
                </p>
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    <form id="converterForm" method="post" enctype="multipart/form-data" class="space-y-6">
                        <div id="uploadContainer" class="upload-area border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer"
                            ...
                            <input type="file" id="textFile" name="textFile" accept=".txt,.text,.log,.csv" class="hidden">
                            <div class="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i class="fas fa-file-alt text-3xl text-blue-500"></i>
                            </div>
                            <h3 class="text-xl font-bold mb-2">Select Text File</h3>
                            <p class="text-gray-500 mb-4">or drag and drop file here</p>
                            <button type="button" onclick="document.getElementById('textFile').click()" 
                                    class="btn-primary text-white px-8 py-3 rounded-lg font-medium inline-flex items-center">
                                <i class="fas fa-folder-open mr-2"></i> Browse Files
                            </button>
                            <p class="text-sm text-gray-400 mt-4">Supports .txt, .text, .log, .csv files up to 50MB</p>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        ...
                        <li><a href="excel_to_pdf.php" class="text-gray-500 hover:text-blue-500 transition">Excel to PDF</a></li>
                        <li><a href="txt_to_pdf.php" class="text-gray-500 hover:text-blue-500 transition">Text to PDF</a></li>
                    </ul>
                </div>
    ...
    <script>
        ...
        // Text to PDF Conversion Functionality
